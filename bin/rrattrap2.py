...
def plot_sp_rated_all(groups, ranks, inffile, ylow=0, yhigh=100, xlow=0, xhigh=120):
    ...
    """
    rank_to_color = {2:'darkgrey', 0:'k', 3:'crimson', 4:'deepskyblue', 5:'indigo', 6:'forestgreen'}
...
#    ppgplot.pgsvp(0.06, 0.97, 0.08, 0.52) # single_pulse_search.py
    ...
    ppgplot.pgsch(0.8)
##adjusted for DGRS since Harper Sewalls is colorblind
...
def main():
    ...
    if options.use_DMplan or options.use_configfile or options.vary_group_size:
        import rrattrap_config
    print "use_DMplan=",options.use_DMplan
    print "use_configfile=",options.use_configfile
    print "vary_group_size=",options.vary_group_size

    RANKS = np.asarray([2,0,3,4,5,6,7])
    
    if options.use_configfile:
	print "Getting configs from rrattrap_config .. "
