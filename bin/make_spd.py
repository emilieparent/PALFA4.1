...
Modification: removed the save_mem argument (subbands data before plotting), as the waterfaller.py version that 
had save_mem implemented can't be found. 
This was necessary because compute nodes on Guillimin had ~2.5 GB of memory.
Beluga's compute nodes have much more than the ~5 GB necessary for to create 
the waterfall plots. 
Also changed the mask in waterfaller.waterfall() from mask = spdcand.mask -> mask = mask
            ...
            for ii in range(len(values)):
                ...
                if plot:
                    ...
                    try:
                        ...
                        print "Error while plotting candidate, skipping to next .." 
