...
def create_download(file_path):
    ...
    query = "INSERT INTO files (" \
            ...
            (filename, file_path, 'downloaded', jobtracker2.nowstr(), \
                jobtracker2.nowstr(), filesize, \
                "Manually added via add_files.py")
    return jobtracker2.query(query)
...
def check_file(fn):
    ...
    if os.path.exists(fn) and os.access(fn, os.R_OK):
        return pipeline_utils2.can_add_file(fn, verbose=True)
    else:
